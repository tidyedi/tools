...
"""List the distinct values seen in each ID (coded) element -- e.g. every
value that showed up in N101, or every REF01 qualifier -- alongside that
element's definition (name, requirement, data type, length), grouped by
transaction set type, the same way :mod:`x12_tools.inventory` groups
segments.

This is deliberately just the *inventory* half of "identify codes we don't
have in our convention": it says what codes actually showed up in the file,
per element position, so they can be read off or exported. Comparing that
against an accepted list -- flagging what's new -- is a separate step, not
implemented here yet, since what "our convention" is checked against (a
...
Only elements whose ``data_type`` is ``"ID"`` in
:data:`x12_tools.element_definitions.SEGMENT_ELEMENTS` are looked at -- a
segment or position not in that reference is simply not covered yet, the
same "blank means not covered" contract as :mod:`x12_tools.segment_names`.
...
#: segment_id -> {position -> definition}, restricted to ID-type elements --
#: computed once from SEGMENT_ELEMENTS rather than maintained as a second,
#: separate reference that could drift from it.
_ID_ELEMENTS: dict[str, dict[int, ElementDefinition]] = {
    segment_id: {e.position: e for e in elements if e.data_type == "ID"}
    for segment_id, elements in SEGMENT_ELEMENTS.items()
}


@dataclass(frozen=True)
class ElementCodes:
    """One ID-type element's definition, plus every distinct value seen at
    that position, in first-appearance order, across every occurrence."""
...
class TransactionSetCodes:
    """One transaction set type's coded-element inventory -- envelope-level
    codes (from ISA/GS) plus this type's own."""
...
@dataclass(frozen=True)
class CodeInventory:
    """One interchange's coded-element inventory."""
...
class _CodeGroup:
    ...
    def add(self, segment_id: str, elements: tuple[bytes, ...]) -> None:
        for position in _ID_ELEMENTS.get(segment_id, {}):
    ...
    def entries(self) -> list[ElementCodes]:
        entries = []
        for sid, pos in self.order:
            definition = _ID_ELEMENTS[sid][pos]
            entries.append(
                ElementCodes(
                    sid,
                    pos,
...
def build_code_inventory(payload: bytes) -> CodeInventory:
    """Walk one cleansed interchange's bytes and collect its coded-element
    values, grouped the same way as :func:`x12_tools.inventory.build_inventory`.
    ...
    envelope_codes = envelope.entries()
    transaction_sets = [
        TransactionSetCodes(ts_id, groups[ts_id].releases, envelope_codes + groups[ts_id].entries())
        for ts_id in group_order
    ]
    return CodeInventory(transaction_sets)

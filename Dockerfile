...
# --- runtime: the venv, the app, and poppler --------------------------------- #
FROM python:3.12-slim AS runtime

# poppler-utils provides pdftotext, which the Convention importer shells out
# to. Every other tool works without it, but the image is small enough
# (~10 MB added) that it's simplest to always have it.
...
CMD ["x12-tools", "serve", "--host", "0.0.0.0"]
